...
var GENERATIONLENGTH = 6; // In seconds
    ...
    for(var i = 0; i < nets.length; ++i){
        ...
        for(var j = 1; j < POSITIONMEMORYSIZE; ++j){
            if(typeof nets[i].inputs[level.length*level[0].length+j*2-2].value === "number"){
                nets[i].inputs[level.length*level[0].length+j*2].value = nets[i].inputs[level.length*level[0].length+j*2-2].value/1.4;
                nets[i].inputs[level.length*level[0].length+1+j*2].value = nets[i].inputs[level.length*level[0].length+j*2+1-2].value/1.4;
            } else {
